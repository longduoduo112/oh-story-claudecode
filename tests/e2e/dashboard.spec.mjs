...
test("用中性夹具浏览拆文库、搜索项目并编辑保存", async ({ page, request }) => {
  const consoleErrors = [];
  ...
  await expect(page.locator("#fileTree")).toContainText("对标样本甲");
  await expect(page.locator("#fileTree")).toContainText("对标样本乙");
  await page.locator(".file-row[data-path='拆文库/对标样本甲/拆文报告.md']").click();
  await expect(page.locator("#editorTitle")).toHaveText("拆文报告.md");
  await expect(page.locator("#editorInput")).toHaveValue(/对标样本甲/);
  ...
  // 搜索由服务端按需扫描，未展开「角色」目录也必须找到顾临；
  // 同时必须真的筛掉不相关的文稿，只断言命中项还在等于放过了「过滤形同虚设」
  await page.locator("#treeSearch").fill("顾临");
...
test("从中性夹具删除文稿前明确确认并刷新文件树", async ({ page, request }, testInfo) => {
  const retryFiles = [
    "拆文库/对标样本甲/_progress.md",
    "拆文库/对标样本甲/快速预览.md",
    "拆文库/对标样本甲/概要.md",
  ];
...
test("保存途中切走文稿，结果不能落到新打开的那份上", async ({ page, request }) => {
  const fileA = "拆文库/对标样本甲/文风.md";
  const fileB = "拆文库/对标样本甲/拆文报告.md";
  await page.goto("/");
  await expect(page.locator("#fileCount")).not.toHaveText("—");
...
test("保存途中继续敲的字仍算未保存", async ({ page, request }) => {
  const filePath = "拆文库/对标样本甲/文风.md";
  await page.goto("/");
  await expect(page.locator("#fileCount")).not.toHaveText("—");
  await page.route("**/api/file", async (route) => {
    if (route.request().method() === "PUT") {
...
test("CRLF 稿件不会被一次改动重写换行，脏标记也能清干净", async ({ page, request }) => {
  const filePath = "长篇/测试长篇项目/设定/文风.md";
...
test("打开文稿不会收起正在翻的目录", async ({ page }) => {
  const rowPath = "长篇/测试长篇项目/大纲/细纲_第002章.md";
  await page.goto("/");
  await page.getByRole("tab", { name: /写作项目/ }).click();
  await expect(page.locator("#fileTree")).toContainText("测试长篇项目");
...
test("@mobile 手机视口仍可从真实长篇项目打开大纲", async ({ page }) => {
  ...
  await page.locator("summary").filter({ hasText: "大纲" }).click();
  await page
    .locator(".file-row[data-path='长篇/测试长篇项目/大纲/大纲.md']")
    .click();
