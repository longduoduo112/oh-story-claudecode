...
- **Windows Hook 路径兼容**：Claude 写后正文网在 Git Bash/MSYS 下先确认正文仍属于项目，再进入项目根，令原生 Node 以当前目录作安全边界、以项目内相对路径读正文；修复书目级/项目级 `.deslop-whitelist` 被误判越界而漏读，同时保留“不读项目外白名单”的安全边界。
