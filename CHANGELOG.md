...
- **Windows Hook 路径兼容**：Claude 写后正文网在 Git Bash/MSYS 下先进入项目根，令原生 Node 以当前目录作安全边界，正文与 CLI 绝对路径继续由宿主统一转换；修复书目级/项目级 `.deslop-whitelist` 被误判越界而漏读，同时保留“不读项目外白名单”的安全边界。
