...
霉味、潮气、声音、光线等通常是角色闻到、听到、看到或感到的对象，不要把它们误写成会「醒来 / 睁眼 / 听见 / 看见 / 闻到 / 感到」的有生主体。正序的「霉味先醒过来」与倒装的「先醒过来的是霉味」是同一类主谓错位。

- **优先改法**：先写感官对象的物理路径（霉味「钻进」鼻腔、脚步声在走廊「响起」、光从门缝「渗进」来），再把醒来、听见、看见等反应还给角色。
- **detector 口径**：`check-ai-patterns.js` 的 `sensory-subject-mismatch` 初期只作 advisory；引号内对话和逐字直接引用（含跨行中文引号块）、书名号及 Markdown inline code 样例豁免。若上下文明确建立了有意拟人，通读确认后可保留或标 `[需复核]`，不机械清零。
...
### 规则 3：句长服从场景（短句和长句都不是默认配额）

叙述（旁白）要让动作、空间和因果自然承接。多数通俗长篇确实常见中等长度、含一至数个逗号的句子，但这只是语料观察，不是逐句配额；高潮、惊惧、喜剧、书信、审讯和不同作者声线都可偏离。短句用作重拍，长句承担连续动作、推理、氛围或情绪链。

| 场景 | 句长 | 示例（长篇语料原句） |
|------|------|------|
| 日常 / 推进 / 描写（多数叙述句） | 读起来能沿动作、空间和因果顺下去 | 阴冷潮湿的气息扑面而来，身下铺着一层薄薄的稻草，湿漉漉地粘在皮肤上。 |
...
> **语料观察**（七猫长篇 现言/都市/古言/玄幻/历史 125 本×前 8 章旁白统计）：逗号之间平均 8.8-9.6 字；整句平均 22-24 字；逗号长句占叙述句 74-80%；≤5 字的短片段约占两成。它只说明“通俗长篇通常不是通篇碎句”，不能要求每句回到该均值。项目已有声音画像时，以本书早期、近期和黄金样本的实际范围定位冷读点。
...
- 标点节奏匹配语气：避免通篇句号化；保留有功能的问号、少量感叹号和人物化停顿；省略号或破折号无功能复现时再改，项目明确禁用时按项目规范处理
- 场景切换有明显标记
- 时间线清晰可追踪
...
### 模式 2：弱化副词泛滥
重点观察：微微、淡淡、缓缓、轻轻。只在同一场景连续替代具体动作、或明显高于本书已接纳/黄金样本时提示；不得用跨题材固定阈值称作“AI 签名”。
...
- 题材文风优先：文风对标有帮助，但必须来自目标题材/本书文风指纹；不要把旧式男频升级文腔、旧网文腔、第一人称声口等当成跨题材万能修法。
...
| `standard` | 命中场景执行 Pass 1 + Pass 2，并做改后残留审计 |
| `aggressive` | 只有用户明确授权结构回炉时，完整三遍 + 重点段落重写 |
...
- [ ] **停顿标点已按语境复核**：`……`/`——` 有人物、节奏或打断功能时可保留；无功能复现、英文点号投机和随机堆砌已清理；项目明确禁用时才按项目模式规范化
- [ ] **AI高频词扫描**：无不禁/仿佛/映入眼帘/心中暗道/沉声道/嘴角微扬/不由自主/只见
- [ ] **弱化副词复核**："微微/淡淡/缓缓/轻轻" 未在同一场景连续替代具体动作；统计只与本书样本对照，不设跨题材硬配额
...
- [ ] **句长实测并冷读**：用脚本报告分布；没有通篇碎句像提纲，也没有臃肿长句压住动作。偏离样本只定位复核，不为回均值机械改写
- [ ] **detector advisory 逐条复核**：`micro-action-tic` / `abstract-summary-tic` / `cliche-density-tic` / `metaphor-density-tic` / `reasoning-chain-tic` / `system-notice-formality-tic` / `overcompressed-prose-tic` / `low-connective-density-tic` / `action-list-tic` / `sensory-subject-mismatch` 命中时按脚本给出的修法处理：先通读判断是不是机械复现或主谓错位，确属再改；功能性写法或有意拟人保留或标 `[需复核]`，不做同义词轮换、不机械注水
- [ ] **不做硬指标投机**：不为反检测强制每句换行、50-60 字一行、对话 50%-60%、英文点省略号，或把 `地/得` 全改成 `的`
- [ ] **任务卡点服从原文边界**：抽象总结若改成角色办事被卡住，必须来自原文已有任务/证据/手续/物件缺口；不新增原文没有的事件链
- [ ] **去AI三遍法服从授权**：`minimal` 只处理有证据局部项，`standard` 处理命中场景，`aggressive` 仅在明确结构回炉授权下执行完整三遍
- [ ] **对话自然度测试**：无书面语痕迹 = 通过
