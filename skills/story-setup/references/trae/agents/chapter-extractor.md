...
<!-- oh-story-managed: agent/chapter-extractor -->
...
未收到 `OUTPUT_MODE: json` 时才使用下方 legacy markdown 格式。收到该标记时，JSON 契约完全取代 markdown 排版契约。

> **结构化输出约束**：调用方可通过 prompt 末尾附加 `OUTPUT_MODE: json` 要求 JSON 格式输出。Stage 2 新任务默认都会带该标记，正常链路不让你直接排版 Markdown。
> 此时，你的最终消息必须是单个 JSON 对象（不带 prose、不带 code fence），结构如下：
> ```
> {
>   "chapter_number": <positive integer>,
>   "title": "<non-empty string>",
>   "summary": "<single-line string, 100-300 Unicode code points，按时序讲清事件/原因/结果>",
>   "key_events": ["<string>"],
...
>     "rhythm_ratio": {"slow_setup": "<0%-100%>", "fast_conflict": "<0%-100%>", "payoff": "<0%-100%>", "hook_space": "<0%-100%>"},
...
>   ],
>   "plot_points": [
>     {"id": "P<integer>", "title": "<string, ≤15 Unicode code points 短标签，不与 event 同句>",
...
>      "quote": "<non-empty string|null, 1-400 Unicode code points；仅关键转折/关键台词/写法样本填，全章至多 8 条>",
>      "quote_locator": "<string|null, 引用过长或分散时改填 5-15 Unicode code points 可 grep 原句片段>",
...
> JSON 模式硬契约（交付前逐条自检）：
>
> - 顶层及所有嵌套 object 都只能有上述 exact keys；不缺字段，不添加解释字段，不输出重复 key、`NaN` 或 `Infinity`
> - `summary` 必须单行，按 Unicode code point（不按 UTF-8 bytes）计 100-300；`rhythm_ratio` 四项必须是 0%-100% 字符串且和为 100%
> - `plot_points` 必须 10-40 项，`id` 严格为 `P1..PN` 无重复、无跳号；`title` ≤15 Unicode code point 且不得等于 `event`
> - `themes` 虽是数组，但必须恰好 1 项且只取主题枚举；所有其他枚举字段也不得自创值
> - 同一情节点的 `quote` 与 `quote_locator` 互斥；全章两者非 null 的情节点合计≤8，quote 非空且为 1-400 Unicode code point，locator 为 5-15
> - 空地点/物品/时间用 `null`，纯环境情节的 `characters` 用 `[]`；无法遵守时只返回 `{"error":"<reason>"}`，不改输 Markdown
...
下列 12 条**同时是主线程的「升级重试」触发条件**：你输出后，主线程会用本清单逐条校验；任一不达标 → 主线程会用 sonnet 覆盖本 agent 的默认 haiku，重新 spawn 一次（仅 1 次）。请在输出前认真自检，不要把校验责任甩给主线程。JSON 模式按字段名检查；legacy Markdown 才检查标签行的冒号与位置。
...
7. 类型标签是否只用了 8 种规定类型
8. `tone` 是否只用了 10 种规定值；legacy Markdown 中还要求分隔符严格为 `基调：`（全角冒号）
9. `themes` 是否恰好一项且只用了 12 种规定值；legacy Markdown 中 `主题标签` 后不加冒号（「温馨/紧张/压抑/甜」等是基调值，禁止填进主题）
