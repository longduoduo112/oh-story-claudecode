...
读取参考文件时，直接 Read 当前 TRAE Code 部署的 canonical 路径，禁止先用 Glob/Grep 搜索：
1. `{项目根}/.trae/skills/story-setup/references/agent-references/{文件名}`
...
父级流程由 TRAE Code 内置 **Agent** 智能体按名称 `character-designer` 调用你。
