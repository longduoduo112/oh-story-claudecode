...
**交付前硬门槛**：交付摘要前自检否定翻转（「不是A，(而)是B」「没有X，没有Y，只是Z」）和中文语言锁，将未获明确授权的英文句段/裸英文词与 HTML 标记清到 0。落盘后由主会话首先跑 `node scripts/language_gate.js <正文文件...>`；返回零后再依次跑 `node scripts/check-ai-patterns.js --check --fail-on=blocking <正文文件...>` 与 `node scripts/check-degeneration.js --check --language=zh --fail-on=blocking <正文文件...>`：`blocking` 命中视为交付未完成，只重写受影响句/段并复扫到 0；外语只有在用户单独确认并精确登记后才可保留。本 agent 默认工具不含 Bash/Node，不得声称已运行脚本，只报告“已按规则自检，等待主会话复扫”。
