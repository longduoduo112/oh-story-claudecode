...
- 合法例外只有：设定已明确的缩写/型号（`AI` / `PDF` / `DB-40`）、URL/邮箱/文件名/代码、用户或设定明确要求逐字保留的外语原文，以及 `.deslop-whitelist` 精确登记的外文专名/短句。
...
- **例外**：西幻 / 科幻 / 都市外企等题材确有外文名时，设定里必须同时有中文译名，正文统一用译名，原文只在首次出现时括注一次。

输出前扫描标题行以外的所有拉丁字母段：整句/整段英文、普通英文词、句首大写英文词与英文临时代号都先改成中文；再核对缩写/型号、URL/邮箱/路径/文件名、代码和白名单例外。不得只扫“全小写且 ≥4 字母”。
...
**交付前硬门槛**：交付摘要前自检否定翻转（「不是A，(而)是B」「没有X，没有Y，只是Z」）和中文语言锁，将未获明确授权的英文句段/裸英文词清到 0。落盘后由主会话依次跑 `node scripts/check-ai-patterns.js --check --fail-on=blocking <正文文件...>` 与 `node scripts/check-degeneration.js --check --language=zh --fail-on=blocking <正文文件...>`：`blocking` 命中视为交付未完成，只重写受影响句/段并复扫到 0；语言类 `advisory` 也必须逐条确认，只有用户/设定明确要求的外语或白名单项才可保留。本 agent 默认工具不含 Bash/Node，不得声称已运行脚本，只报告“已按规则自检，等待主会话复扫”。
...
- 初稿完成后必须运行 story-deslop 的 `language_gate.js`，不得直接进入下一阶段。
- Gate 返回非零时，接收行号、原片段和所在行，结合上下文修改原句或原段，然后重新运行 Gate。
- Gate 返回码为零之前，本章不得验收、提交追踪或开始下一章。
- 禁止用自动翻译、简单删除或新增白名单绕过退回修改。
- 缩写、型号和剧情代号不得自行放行；只有检测器机械保护的非叙事 URL、邮箱、代码、文件名，以及用户明确要求保留的内容例外。

## 对白归属标记退回机制

- 中文语言 Gate 通过后运行 story-deslop 的 `dialogue_drift_gate.js`。
- Gate 退回时，根据行号和上下文重写高密度“谁说/谁问”，然后复检。
- 轮流对白清楚时可省略标记；需要承接时使用真正推动场景的动作、反应和人物化语气。
- 不得机械替换成“沉声道/冷声道/缓缓说道”，也不得给每句对白强塞无意义动作。
- Gate 通过前不得提交追踪或开始下一章。
