...
2. **细纲必填项**：每个细纲（细纲_*.md）必须包含当前“章节蓝图”：
   ...
   - 复沓锚句：无，或逐条精确登记必须在正文原样回环的誓言、系统提示、案卷引文；普通概括语和整段细纲不得列入豁免
   - 基础执行字段：本章核心事件、字数目标、目标情绪、单元ID/位置、主角目标/关键选择、章首拉力、本章爽点（可无显性）、本章兑现、状态变化、章尾余势/钩子、契约风险（逐章按大纲安全七检⑥⑦复核后填写）
   - 内容概括（五段式）：起因 / 发展 / 转折 / 高潮 / 结尾——结尾写最后落在谁的什么具体动作、画面或台词上，不写状态判词
   ...
   - 结尾设定和钩子：具体落幕动作/画面/台词、未解决问题、下一章推动力、章尾余势；硬钩子为可选策略

3. **增量写入**：大纲必须增量写入——先写骨架（关键事件和爽点），再逐节填充细节。不要一次性写出所有细节。

4. **与正文同步**：写完一章正文后，回头更新细纲中"实际完成情况"部分。

5. **章首必须有进入本章的拉力，但不必章章硬钩子**（完整技法见 `story-setup/references/agent-references/hooks-chapter.md`）：
   高压/推进章可选 悬念对话开局 | 闪前碎片 | 倒计时开局 | 神秘独白 | 反差场景 | 未完成动作开局 | 意象预示；低压/关系章可直接承接上一章行动、关系或情绪余势。

6. **每章执行“收一个、变一个、开一个”**：先兑现一笔期待或付利息，再让目标/风险/信息/关系/资源/身份/情绪立场至少一项改变，最后留下问题、决定、行动、关系变化、阶段目标或情绪余势。高压节点需要硬钩子时可选：
   突然揭示 | 紧急危机 | 未完成动作 | 身份反转 | 两难抉择 | 神秘物品/线索 | 倒计时 | 承诺/威胁 | 离奇消失 | 隐藏含义 | 意象钩子 | 回声钩子 | 留白钩子。低压章无硬悬念不算缺项；下一章必须真实承接，不得撤回、误会化或切线逃债。
...
## 本卷反转
第18章：暗卫首领竟是林晚失散多年的兄长（身份反转，误导路径：首领屡次针对林晚）
```
