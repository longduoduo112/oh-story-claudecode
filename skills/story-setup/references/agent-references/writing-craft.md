...
```
❌ 低密度（1 件事/100 字）：
「林晚是沈家的嫡女，自幼聪慧，深得父亲喜爱。这一天，她收到了一道圣旨。」
...
不同题材的真实文风 cadence 会影响读感；有效写法不同。男频/军旅可以借旧网文连贯叙事惯性，现代日常可写普通任务推进；悬疑、科幻、古代不要硬套旧式男频升级文腔、旧网文腔或第一人称声口，先读本书 `设定/文风.md`、对标拆文和匹配章摘要。
...
标点承担语气、权力关系和情绪节拍，不能当装饰。写正文时先判断“这句话在场景里做什么”，再决定用句号、逗号、问号、感叹号、冒号、换行/动作 beat，或少量有功能的省略号、破折号。标点本身不是 AI 身份证据；重复用同一停顿标记替代动作、潜台词或节拍变化，才需要处理。
...
| 犹豫 / 未说完 / 心虚 | 逗号、句号、短句、吞咽/停手等动作；必要时 `……` | 优先用动作或句长变化停顿；省略号只保留人物确实吞回话、拖音或悬置的少数位置 | 不把每次犹豫都写成 `……` |
| 被打断 / 拖长音 | 动作打断、换行、短句、未完成动作；必要时 `——` | 用动作切断最清楚；角色话语被截断、声线确有拖长时可保留破折号 | 不用破折号替代所有动作节拍，不堆叠 `--` |
...
3. 复核正文（含对话）里的 `……`、`——`、`—`、`--`：有明确的吞咽、未尽、截断、拖音或声线功能时保留；无功能复现、英文双连字符投机、随机堆叠，或项目/平台明确禁用时，改成句号、逗号、短句、换行或动作断句。知乎盐言 `「」` 只是引号风格，不影响其他标点的合法使用。
