...
description: |
  ...
  WebSearch/WebFetch 作为兜底。输出带来源引用的结构化 Markdown 参考文件。
  被 story-long-write（Phase 4）、story-review、story skill 路由调用。
tools: Read, Glob, Grep, Bash, Write, WebFetch, WebSearch
disallowedTools: Edit
---

<!-- oh-story-managed: agent/story-researcher -->
...
3. WebSearch / WebFetch → 兜底（CDP 不可用或页面打不开时）
```
...
- `CDP_UNAVAILABLE` → 直接降级到 WebSearch/WebFetch
...
- Bing 也失败 → 降级到 WebSearch/WebFetch 兜底
...
如果 CDP 和 WebSearch 均不可用（如 WebSearch 配额耗尽、WebFetch 返回错误）：
...
父级流程由 WorkBuddy / CodeBuddy Code 内置 **Agent** 智能体按名称 `story-researcher` 调用你。
