...
> **语料读取**：本阶段及阶段 4a/4c、散落情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md`（Stage 2 收尾按章号无损拼接的全书摘要，见 SKILL.md「Stage 2 收尾：合并章节摘要」）**一次性读取、在上下文中复用**，不逐文件 glob 重扫——同一份语料的 4-5 次冷读降为 1 次。汇总文件缺失或语料超上下文时，按本文「处理批次」执行：>500 章默认由子代理按批读原始摘要并回传降维聚合，主线程不回退到逐章扫描。
...
| 类型 | 定义 | 可合并 | 示例 |
|------|------|--------|------|
| proper_name | 专名/全名/常用名 | 是 | "罗岑""顾临川" |
| nickname | 绰号/外号，须有同指证据 | 是（置信度≥0.85） | "赤纹战士顾临川" |
...
> **先分清两个互不相干的轴**，不要把它们都简写成「块」：
> - **语义分块**＝*在哪里切*。按叙事弧切，产出「块元数据」用于组织分析结果，块可以很大（一部 914 章的书通常 5-10 个弧）。它不决定一次读多少。
> - **处理批次**＝*一次读多少*。受上下文容量约束，由摘要体积反推章数。下表「块大小」列指的是**处理批次**。
...
Stage 3-5 的语料是 `章节/*_摘要.md`。中文章节摘要实测约 15KB/章，约 5-7K tokens/章；在 >500 章时，把全书摘要一次读进主线程必然超出上下文。大书必须换成**子代理并行 + 降维聚合**，不是把语义块调大。

**A. 子代理并行模式（>500 章默认，Stage 3-5 通用）**

1. **切批**：按章号顺序切成 10-20 章/批（约 5-15 万 tokens/批，落在子代理单次上下文内）。单章摘要明显更长时，按「一批不超过约 15 万 tokens」反推章数。批次边界尽量对齐语义块边界，对不齐时以章号连续为准。
2. **并行分派**：每批 spawn 一个子代理（`general-purpose` 或本阶段对应的专业 agent），把该批摘要文件路径交给它，**由子代理在自己的上下文里读原始摘要**，主线程不读。
...
5. **进度与按批恢复**：每批完成后更新 `_progress.md`，记录批次号、覆盖章节区间、聚合产出路径和状态。中断后只从第一个未完成批次续跑，已完成批次的聚合产物直接复用。

> 覆盖示例：914 章按 15 章/批约 61 批；主线程全程只处理 61 份 ≤8K tokens 的聚合，不触碰原始语料。

**B. 主线程串行模式（agent 不可用时的降级）**

无法 spawn 子代理时退回主线程顺序处理：**输入 6-8K token/块、章节边界对齐**，逐块读取并滚动累积中间结论（每块处理完只保留结论、丢弃原文）。这是「6-8K token/块」规格的**唯一适用场景**；它描述的是主线程单次读取量，不是大书的语义块尺度。串行模式对 >500 章耗时很长，但结论质量与并行模式一致。
...
- 进度通过 `_progress.md` 追踪，>500 章的 Stage 3-5 须记录处理批次与聚合产出路径
- 新会话读 `_progress.md` 定位第一个未完成批次
- 按批恢复：复用已完成批次的聚合产物，只重跑未完成/失败批次；不因新会话重读全书摘要
