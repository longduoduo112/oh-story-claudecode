...
新 Stage 2 任务不让模型直接排版 Markdown。并行 chapter-extractor 与 solo/direct 串行降级都先产出下面的**单个纯 JSON object**（无 prose、无 code fence），再交给 `scripts/render_chapter_summary.py`。以下是人类可读 schema；validator 是机器权威。

```text
{
  "chapter_number": <positive integer>,
  "title": "<non-empty string>",
  "summary": "<single-line string, 100-300 Unicode code points>",
  "key_events": ["<non-empty string>", "..."],
  ...
  "chapter_formula": {
    "emotion_flow": {
      ...
      "turn": "<string>", "close": "<string>"
    },
    "rhythm_ratio": {
      "slow_setup": "<0%-100%>", "fast_conflict": "<0%-100%>",
      "payoff": "<0%-100%>", "hook_space": "<0%-100%>"
    },
    "structure_formula": ["<string>", "..."],
    "core_technique": "<string>",
    "hook_and_foreshadowing": "<string>"
  },
  "characters": [
    {
      "name": "<string>",
  ...
  "plot_points": [
    {
      "id": "P<integer>",
      "title": "<1-15 Unicode code points>",
      "event": "<objective event string, different from title>",
      "type": "转折点|信息揭示|冲突|解决|铺垫|行动|对话|状态变化",
      "characters": ["<string>"],
      "location": "<string>" | null,
      "item": "<string>" | null,
      "time": "<string>" | null,
      "quote": "<1-400 Unicode code points>" | null,
      "quote_locator": "<5-15 Unicode code points>" | null,
...
- 顶层及所有嵌套 object 都是 exact keys；不接受缺字段、额外字段、重复 JSON key、`NaN` / `Infinity`
- `summary` 按 Python `len(str)` 计 Unicode code point（不按 UTF-8 bytes），必须单行 100-300；`rhythm_ratio` 四项必须是 0%-100% 字符串且和为 100%
- `plot_points` 数量 10-40，`id` 严格为 `P1..PN` 无重复、无跳号；`themes` 虽为数组但必须恰好 1 项
- `quote` 与 `quote_locator` 互斥；全章两者非 null 的情节点合计≤8，quote 非空且单条为 1-400 Unicode code point，locator 为 5-15
- 空地点/物品/时间用 `null`；纯环境情节的 `characters` 用 `[]`；章号与标题必须与调用任务相符
- 解析/校验失败不得碰已有 Markdown；升级 JSON 重试仍失败后才允许明示 `legacy_markdown_fallback`；已落盘的旧 Markdown 不追溯

### Stage 2 Markdown 投影

下面是 renderer 的确定性投影格式，同时作为两次 JSON 失败后明示 legacy fallback 的排版契约。新任务的模型正常路径不直接输出本节 Markdown。
...
P{序号} **{标题}**：类型{转折点/信息揭示/冲突/解决/铺垫/行动/对话/状态变化} | {白描一句话：谁做了什么、结果如何；原文给出起因或理由的一并写进来，不推测动机；埋伏笔的写出伏笔线索} | 涉及{全名，多人逗号分隔；纯环境铺垫无具体人物时填无} | 地点{如明确；否则填无} | 物品{如涉及；否则填无} | 时间{如明确；否则填无}
...
| 评价 | 这是一段精彩的打斗 | 顾临川三招击败对手，围观者倒吸一口凉气 |
...
**Stage 2 语义自检**（并行 agent 与 solo/direct 串行两条路径同一份要求；格式项由 JSON validator/renderer 机械保证）：概要为按时序的连贯叙述（非条目罗列、非同一连接词反复串联）｜每个情节点白描客观且写全事实/结果/原文给出的起因｜标题为 ≤15 Unicode code point 短标签且不与白描同句｜情节点严格按时序｜情节点数 10-40（按字数动态调节，硬下限 10）｜关键转折/关键台词/写法样本类情节点留了 `quote` 或 `quote_locator`，全章不超过 8 条｜角色用全名｜角色描述不跨章｜`key_information_expansion` 与 `chapter_formula` 信息齐全。

> 上文「落盘文本无花括号 / 空字段写无 / 标签行位置」等 Markdown 规则是 renderer 投影规范和 legacy fallback 自检，不是要求 JSON 模式的模型直接排版。legacy fallback 仍要求「落盘文本无 `{`/`}`」；已落盘旧 Markdown 不追溯。
