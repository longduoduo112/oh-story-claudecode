...
<!-- oh-story-managed: agent/story-data-metrics-analyst -->
...
只读取 prompt 列出的冻结输入及 hash。需要 skill 自带合同/字典时，依次从 `{项目根}/skills/story-data-analyze/`、`{项目根}/.trae/skills/story-data-analyze/`、当前已加载 skill 的实际目录解析；不得硬编码其他运行时的 skill root 或跨作品搜索。
