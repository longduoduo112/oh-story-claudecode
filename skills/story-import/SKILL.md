...
本步只处理 Phase 1 显式绑定的外部参考作品。把 `拆文库/{对标书名}/` 的结构化分析资产同步到项目引用视图 `{项目}/对标/{对标书名}/`，供 story-long-write 优先读取。没有绑定外部对标时跳过本步，不创建空目录；严禁使用 `拆文库/{导入书名}/` 或项目 `设定/` 作为复制源。

| 源路径 | 项目对标路径 | 用途 |
|-------|-------------|------|
| `拆文库/{对标书名}/剧情/节奏.md` | `{项目}/对标/{对标书名}/剧情/节奏.md` | story-long-write 读取关键信息推进、情绪触动点、爆发节奏 |
| `拆文库/{对标书名}/剧情/情绪模块.md` | `{项目}/对标/{对标书名}/剧情/情绪模块.md` | story-long-write 读取读者需求 / 情绪引擎、可复现模块 |
