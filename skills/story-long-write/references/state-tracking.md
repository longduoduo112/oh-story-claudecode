...
角色快照只回答“此刻怎样”，不承担整书的身世、血缘、物权、权限和不可逆规则。这些长期硬事实存在 `_tracking-state.json.facts`，由工具派生：

- `追踪/长期事实.md`：所有稳定断言的当前值；
- `追踪/关系清单.md`：血缘、亲属、婚姻、传承、从属、合作、敌对等关系投影；
- `追踪/事实档案/{实体}.md`：某个人/机构/核心物件相关的有界事实包。

续写时先从细纲提取本章实体，只读对应事实档案；需要查事实如何形成时，再沿 `evidence` 定点读原始设定/正文章。不用会被 compact 的聊天记忆、不用角色快照中一句模糊摘要代替原始证据。

收录与修订门禁见当前写作流程的“跨产物修订与长期事实”协议；JSON 字段见 [tracking-transaction.md](tracking-transaction.md)。所有事实/关系 Markdown 都禁止手改。
