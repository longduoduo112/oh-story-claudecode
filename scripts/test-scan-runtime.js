...
function testLocalDateStamp(modulePath) {
  ...
  assert.strictEqual(typeof utils.captureRunClock, "function");
  ...
  const fixed = new Date(2026, 6, 27, 0, 30);
  const clock = utils.captureRunClock(fixed);
  assert.strictEqual(clock.utc, fixed.toISOString());
  assert.strictEqual(clock.localDate, "20260727");
  assert(Object.isFrozen(clock), "run clock must be immutable");
...
function testScraperFilenameDatesAreLocal() {
  for (const scraperPath of listScraperPaths()) {
    ...
    assert(
      src.includes("captureRunClock()") && src.includes("RUN_CLOCK.localDate"),
      `${name}: 报告头与输出文件名必须复用同一个本地日期时钟`
    );
    assert(
      src.includes("RUN_CLOCK.utc"),
      `${name}: UTC 抓取时刻也必须来自同一个 run clock，不能在渲染时再次取时`
    );
    assert(src.includes("抓取时间（UTC）"), `${name}: 文件头必须明确 UTC 时间口径`);
    assert(src.includes("报告日期（本地）"), `${name}: 文件头必须明确文件名使用的本地日期`);
  }
}
...
function testQimaoPeriodPlan() {
  ...
  assert.match(incomplete, /抓取时间（UTC）：2026-08-11T00:00:00\.000Z/);
  assert.match(incomplete, /报告日期（本地）：20260811/);
