...
assert_grep 'language_gate\.js' "$SKILL_DIR/references/templates/agents/narrative-writer.md" "narrative-writer must hand off the standalone language gate first"
...
assert_grep '用户单独确认' "$SKILL_DIR/references/templates/agents/narrative-writer.md" "narrative-writer whitelist must require separate user confirmation"
assert_grep 'HTML 标签、注释和实体|HTML 标记' "$SKILL_DIR/references/templates/agents/narrative-writer.md" "narrative-writer must block HTML in delivery prose"
...
for template in generic openclaw reasonix; do
  assert_grep 'node "\{当前写作 Skill 目录\}/scripts/language_gate\.js" "\{正文文件\}"' "$SKILL_DIR/references/$template/AGENTS.md.tmpl" "$template skills-only deployment must actually run standalone language gate"
  ...
  assert_grep '用户单独确认' "$SKILL_DIR/references/$template/AGENTS.md.tmpl" "$template skills-only deployment must require separate user confirmation for whitelist entries"
  assert_grep 'HTML' "$SKILL_DIR/references/$template/AGENTS.md.tmpl" "$template skills-only deployment must block HTML in delivery prose"
